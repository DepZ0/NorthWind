...
export class ProductsDb {
  ...
  public getAllProducts = async (page: number, pageSize: number) => {
    ...
    const query = this.db.select().from(products).limit(pageSize).offset(offset).toSQL();
  ...
  public getProductById = async (id: number) => {
    ...
    const query = this.db.select().from(products).where(eq(products.productId, id)).toSQL();
    const res = await this.pool.query(`EXPLAIN ANALYZE ${query.sql}`, query.params);
    const duration = getExecutionTime(res);
